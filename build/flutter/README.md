...
* `fuelmix` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `fuelmix` - executable or bundle name on disk.
* `FuelMix` - display name shown in window titles and about app dialogs.
* `com.fuelmix` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
* `FuelMix` - the name of the company.
